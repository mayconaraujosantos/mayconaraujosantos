...
```kotlin
package com.bank.guarantees.card.receivables.domain.dataaccess

import com.bank.guarantees.card.receivables.domain.entities.CardReceivablesLock
import com.bank.guarantees.card.receivables.domain.entities.Person
...
class CardReceivablesLockDataAccessImplTest {
  private lateinit var repository: CardReceivablesLockRepository
  private lateinit var dataAccess: CardReceivablesLockDataAccessImpl

  @BeforeEach
  fun setUp() {
    repository = mockk<CardReceivablesLockRepository>()
    dataAccess = CardReceivablesLockDataAccessImpl(repository)
  }

  private fun createPerson(id: Long = 1L, name: String = "John Doe"): Person {
  ...
  private fun createCardReceivablesLock(
          ...
          ipoc: String = "IPOC001",
          ownerPerson: Person? = createPerson(),
  ...
  fun `should create card receivables lock successfully`() {
    ...
    every { repository.save(lock) } returns savedLock

    // When
    val result = dataAccess.create(lock)

    // Then
    assertEquals(savedLock, result)
    verify(exactly = 1) { repository.save(lock) }
  }
  ...
  fun `should update card receivables lock successfully`() {
    ...
    every { repository.save(lock) } returns updatedLock

    // When
    val result = dataAccess.update(lock)
  ...
  fun `should get card receivables lock by id when exists`() {
    ...
    every { repository.findById(lockId) } returns java.util.Optional.of(lock)

    // When
    val result = dataAccess.getById(lockId)

    // Then
    assertEquals(lock, result)
    verify(exactly = 1) { repository.findById(lockId) }
  }
  ...
  fun `should return null when card receivables lock not found by id`() {
    // Given
    val lockId = 999L
    every { repository.findById(lockId) } returns java.util.Optional.empty()
  ...
  fun `should increment creation retry attempts successfully`() {
    ...
    // When
    val result = dataAccess.incrementCreationRetryAttempts(lockId)
  ...
  fun `should increment proactive search attempts successfully`() {
    ...
    // When
    val result = dataAccess.incrementProactiveSearchAttempts(lockId)
  ...
  fun `should find card receivables lock by contract number when exists`() {
    ...
    every { repository.findByContractNumber(contractNumber) } returns lock

    // When
    val result = dataAccess.findByContractNumber(contractNumber)
  ...
  fun `should return null when card receivables lock not found by contract number`() {
    ...
    every { repository.findByContractNumber(contractNumber) } returns null
...
@Repository
interface CardReceivablesLockRepository : JpaRepository<CardReceivablesLock, Long> {
...
data class CardReceivablesLock(
        ...
        val ipoc: String,
        @ManyToOne @JoinColumn(name = "owner_person_id") val ownerPerson: Person? = null,
        val amount: Double,
...
CREATE TABLE card_receivables_lock (
    ...
    CONSTRAINT fk_card_receivables_lock_person FOREIGN KEY (owner_person_id) REFERENCES person (id)
);
